...
export async function checkRepository(root, { exerciseRuntime = true, exerciseHttp = true, codexExecutable, spawnCodex } = {}) {
  ...
  const dashboardPath = path.join(root, 'preview', 'index.html'), dashboardModulePath = path.join(root, 'preview', 'artifact-tabs.js'), networkPath = path.join(root, 'preview', 'agent-network.js');
  const dashboardExists = existsSync(dashboardPath), dashboardModuleExists = existsSync(dashboardModulePath), networkExists = existsSync(networkPath);
  ...
  if (exerciseRuntime) {
    ...
    report.require(JSON.stringify(state.progress) === JSON.stringify({ completed: 1, total: 1 }), 'Runtime mock transitions pass', 'runtime derived progress is incorrect'); report.require(state.active_agents.length === 0 && state.signals.length === 2, 'Agent lifecycle and signals pass', 'agent lifecycle state is incorrect'); await store.upsertTask('orphan', 'Orphan test', 'pending', 'missing-agent'); const warnings = inspectRuntime(await store.readSnapshot()); report.require(warnings.some(item => item.id === 'unknown-owner-orphan'), 'Watchdog mock transition passes', 'watchdog smoke test failed'); await store.upsertTask('orphan', 'Orphan test', 'cancelled', 'missing-agent'); const staticHtml = renderStaticPreview(await store.readSnapshot(), [], { agents: [] }, networkExists ? readFileSync(networkPath, 'utf8') : ''); report.require(staticHtml.includes('Write Claims'), 'Static preview generation passes', 'static preview generation failed'); report.require(networkExists && staticHtml.includes('Agent Signal Network') && staticHtml.includes('Snapshot History'), 'Static agent network generation passes', 'static agent network generation failed');
      if (exerciseHttp) { const server = await createPreviewServer(temporary); await new Promise(resolve => server.listen(0, '127.0.0.1', resolve)); try { const port = server.address().port, html = await (await fetch(`http://127.0.0.1:${port}/`)).text(), moduleResponse = await fetch(`http://127.0.0.1:${port}/preview/artifact-tabs.js`), moduleSource = await moduleResponse.text(), payload = await (await fetch(`http://127.0.0.1:${port}/runtime/snapshot`)).json(), networkResponse = await fetch(`http://127.0.0.1:${port}/preview/agent-network.js`), networkSource = await networkResponse.text(); report.require(html.includes('Watchdog Warnings') && payload.status.current_goal === 'Mock SMALL task' && payload.status.task_counts.total === payload.tasks.tasks.length && payload.claims.claims.length === 1, 'Dashboard serves a consistent runtime snapshot', 'dashboard snapshot smoke test failed'); report.require(moduleResponse.ok && /text\/javascript/.test(moduleResponse.headers.get('content-type') || '') && moduleSource.includes('renderArtifactTabs'), 'Dashboard module is served', 'dashboard module smoke test failed'); report.require(networkResponse.ok && /text\/javascript/.test(networkResponse.headers.get('content-type') || '') && networkSource.includes('AgentSignalNetwork'), 'Agent network renderer is served', 'agent network renderer smoke test failed'); } finally { await new Promise(resolve => server.close(resolve)); } }
  }
  return report;
}
