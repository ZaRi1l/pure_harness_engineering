...
export class RuntimeStore {
  ...
  async agentStarted(id, role, task = '', metadata = {}) { await this.mutate(({ status, events }) => { const agent = { id, role, status: 'running', current_task: String(task).slice(0, 300), started_at: now(), stopped_at: null }; status.active_agents = status.active_agents.filter(item => item.id !== id).concat(agent); status.agents = status.agents.filter(item => item.id !== id).concat({ ...agent }).slice(-30); status.signals = status.signals.concat({ time: now(), from: 'main', to: id, kind: 'delegate', summary: agent.current_task || `Start ${role}`, ...signalMetadata({ task_id: metadata.task_id }) }).slice(-50); events.push(this.event('agent_started', `${role} started`, { agent_id: id })); }); }
  async agentStopped(id, outcome = 'stopped', metadata = {}) { await this.mutate(({ status, events }) => { const active = status.active_agents.find(agent => agent.id === id); status.active_agents = status.active_agents.filter(agent => agent.id !== id); for (const agent of status.agents) if (agent.id === id) Object.assign(agent, { status: outcome, stopped_at: now() }); status.signals = status.signals.concat({ time: now(), from: id, to: 'main', kind: 'result', summary: String(outcome).slice(0, 300), ...signalMetadata({ task_id: metadata.task_id, status: outcome !== 'stopped' ? outcome : undefined }) }).slice(-50); events.push(this.event('agent_stopped', `${active?.role || 'agent'} ${outcome}`, { agent_id: id })); }); }
  async addSignal(from, to, kind, summary, metadata = {}) { await this.mutate(({ status, events }) => { status.signals = status.signals.concat({ time: now(), from, to, kind: String(kind).slice(0, 80), summary: String(summary).slice(0, 300), ...signalMetadata(metadata) }).slice(-50); events.push(this.event('agent_signal', `${from} -> ${to}: ${kind}`)); }); }
