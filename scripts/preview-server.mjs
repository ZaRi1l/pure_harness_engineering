...
export async function createPreviewServer(root, host = '127.0.0.1') {
  ...
  return http.createServer(async (request, response) => {
    try {
      ...
      const runtime = { '/runtime/snapshot': () => store.readSnapshot(), '/runtime/status': () => store.readStatus(), '/runtime/tasks': () => store.readTasks(), '/runtime/events': async () => ({ events: await store.readEvents() }), '/runtime/catalog': () => discoverCatalog(root) };
