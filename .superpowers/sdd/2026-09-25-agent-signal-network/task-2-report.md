...
## Final legacy overlap correction

- A retained unique id-less signal remains selected when one update both prunes older signals and appends newer ones. The renderer now finds the longest old-suffix/new-prefix overlap and retains selection only when the selected edge lies in that overlap. Duplicate-count changes still clear selection; the existing duplicate-replacement regressions remain green.
- TDD evidence: the new `[A,B,C] -> [B,C,D]` regression made `node tests/agent-network.test.mjs` exit 1 with 16/17 passed because selected B became null. After the overlap fix, the same focused command exited 0 with 17/17 passed.
- No runtime source or tests changed in this correction. The full suite was intentionally not rerun while its separate Windows `EPERM` event-history failure is being stabilized.
